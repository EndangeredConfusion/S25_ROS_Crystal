...
class TurtleClient(Node):
    def __init__(self):
        ...
        # Creating turtleColor parameter
        self.declare_parameter('turtleColor', 'red', 
        ParameterDescriptor(description='Controls the color of the turtle.'))
        ...
        while not self.color_cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("Color service not availiable, waiting...")
        ...
        # Thickness default argument from step 14.
        self.declare_parameter("penThickness", 7, ParameterDescriptor(description="sets thickness of turtle pen"))
        thickness = self.get_parameter("penThickness").get_parameter_value().integer_value
        self.turtle_display.pensize(thickness)
